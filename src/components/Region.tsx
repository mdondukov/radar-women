...
import {Loader, NextButton} from "./index";
import {useStores} from "../hooks/use-stores";
import {fetchRegions} from "../http/api";
...
export const Region: React.FC = observer(() => {
    ...
    React.useEffect(() => {
        ...
        fetchRegions(uiStore.locale, uiStore.secondaryLocale).then(response => {
    ...
    React.useEffect(() => {
        if (regionStore.selectRegion.regionId > -1 && regionStore.selectRegion.areaId > -1) {
            stepStore.setCompleteStep(stepStore.activeStep.id)
        }
    }, [regionStore.selectRegion, stepStore])

    if (regionStore.isLoading) {
        return <Loader/>
    }

    if (regionStore.error) {
        throw Error(regionStore.error)
    }

    const region1 = regionStore.getRegionIdByCode("region-1")
    const region2 = regionStore.getRegionIdByCode("region-2")
    const region3 = regionStore.getRegionIdByCode("region-3")
    const region4 = regionStore.getRegionIdByCode("region-4")
    const region5 = regionStore.getRegionIdByCode("region-5")
    const region6 = regionStore.getRegionIdByCode("region-6")
    const region7 = regionStore.getRegionIdByCode("region-7")
            ...
            <div className="w-full relative">
                ...
                </div>
                {
                    regionStore.selectRegion.regionId > -1 && (
                        <div
                            className={getModalClass(regionStore.getRegion(regionStore.selectRegion.regionId).ord, uiStore.windowDimensions.width)}
                        >
...
const isRegionSelect = (regionId: number, selectRegion: IRegionSelect): boolean => {
    return selectRegion.regionId === -1 || selectRegion.regionId === regionId
}

const getModalClass = (ord: number, windowWidth: number): string => {
